...
class HighScore:
    ...
    def __cmp__(self, other):
        c = (self.score > other.score) - (self.score < other.score)
        if c == 0:
            return (other.date > self.date) - (other.date < self.date)
        else:
            return c
...
class EntrySequenceManager(game.Mode):
    ...
    def create_highscore_entry_mode(self, left_text, right_text, entered_handler):
        ...
        return InitialEntryMode(game=self.game, priority=self.priority + 1, left_text=left_text, right_text=right_text,
