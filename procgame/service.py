...
class CoilTest(ServiceModeList):
    ...
    def mode_started(self):
        ...
        self.action = 'manual'
        if self.game.lamps.has_key('startButton'): self.game.lamps.startButton.schedule(schedule=0xff00ff00,
    ...
    def __init__(self, game, priority, font):
        ...
        for switch in self.game.switches:
            if self.game.machine_type == 'sternWhitestar':
                ...
                add_handler = 0
            if add_handler:
                self.add_switch_handler(name=switch.name, event_type='inactive', delay=None,
...
class SettingsEditor(ServiceModeList):
    ...
    def __init__(self, game, priority, font, name, itemlist):
        ...
        self.layer = GroupedLayer(128, 32,
                                  [self.title_layer, self.item_layer, self.value_layer, self.instruction_layer])
        for item in sorted(itemlist.iterkeys()):
            # self.items.append( EditItem(str(item), itemlist[item]['options'], itemlist[item]['value'] ) )
            if 'increments' in itemlist[item]:
                num_options = (itemlist[item]['options'][1] - itemlist[item]['options'][0]) / itemlist[item][
                    'increments']
                option_list = []
                ...
                self.items.append(
                    EditItem(str(item), itemlist[item]['options'], self.game.user_settings[self.name][item]))
        self.state = 'nav'
