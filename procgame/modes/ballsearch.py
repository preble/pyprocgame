...
class BallSearch(Mode):
    ...
    def __init__(self, game, priority, countdown_time, coils=[], reset_switches=[], stop_switches=[],
                 enable_switch_names=[], special_handler_modes=[]):
        ...
        for switch in reset_switches:
            self.add_switch_handler(name=str(switch), event_type=str(reset_switches[switch]), delay=None,
                                    handler=self.reset)
        ...
        for switch in stop_switches:
            self.add_switch_handler(name=str(switch), event_type=str(stop_switches[switch]), delay=None,
                                    handler=self.stop)
    ...
    def reset(self, sw):
        if self.enabled:
            ...
            if schedule_search:
                ...
                self.delay(name='ball_search_countdown', event_type=None, delay=self.countdown_time,
                           handler=self.perform_search, param=0)
    ...
    def perform_search(self, completion_wait_time, completion_handler=None):
        ...
        for coil in self.coils:
            ...
            delay = delay + .150
        self.delay(name='start_special_handler_modes', event_type=None, delay=delay,
    ...
    def start_special_handler_modes(self):
        for special_handler_mode in self.special_handler_modes:
            ...
            self.delay(name='remove_special_handler_mode', event_type=None, delay=7,
                       handler=self.remove_special_handler_mode, param=special_handler_mode)
