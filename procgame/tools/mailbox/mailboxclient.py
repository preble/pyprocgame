...
class MailboxClient:
    ...
    def poll_for_result(self, job_key):
        ...
        conn = self.connection()
        conn.request('GET', '/jobs/%s/result?%s' % (job_key, urllib.parse.urlencode(params)))
        resp = conn.getresponse()
        resp_data = resp.read()
        conn.close()
    ...
    def list_jobs(self, timestamp):
        ...
        conn = self.connection()
        conn.request('GET', '/jobs/list?' + urllib.parse.urlencode(params))
        resp = conn.getresponse()
        resp_data = resp.read()
        conn.close()
        if resp.status == 200:
    ...
    def get_job_input(self, job_key):
        params = {'api_key': self.api_key}
        conn = self.connection()
        conn.request('GET', '/jobs/%s/request?%s' % (job_key, urllib.parse.urlencode(params)))
        resp = conn.getresponse()
        resp_data = resp.read()
        conn.close()
        if resp.status == 200:
