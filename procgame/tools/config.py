...
def save_config():
    try:
        ...
        if not os.path.isdir(dirname):
            ...
            os.makedirs(dirname)
        output = open(procgame.config.path, 'w')
    ...
    if options.key:
        ...
        if options.add or options.remove:
            if options.key in procgame.config.values and type(procgame.config.values[options.key]) != list:
                print("Cannot add or remove values from a key that does not have a list value.  Type is " + (
        ...
        if options.set:
            if options.key in procgame.config.values and type(procgame.config.values[options.key]) != str:
                print("Cannot assign a value to a key that is not a string.  Type is " + (
                    type(procgame.config.values[options.key]).__name__))
                sys.exit(3)
