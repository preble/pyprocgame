...
def load():
    ...
    if not os.path.exists(path):
        print('procgame.config: WARNING pyprocgame configuration not found at %s' % path)
        return
    # print('procgame.config: pyprocgame configuration found at %s' % path)
    try:
        values = yaml.load(open(path, 'r'))
    except yaml.scanner.ScannerError, e:
        print('procgame.config: ERROR loading pyprocgame config from %s; your configuration file has a syntax error in it!\nDetails: %s' % (path, e))
    except Exception, e:
        print('procgame.config: ERROR loading pyprocgame config from %s: %s' % (path, e))
