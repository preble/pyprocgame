...
class TextLayer(Layer):
    ...
    def next_frame(self):
        ...
        if (self.seconds is not None) and ((self.started_at + self.seconds) < time.time()):
            self.frame = None
...
class PanningLayer(Layer):
    ...
    def next_frame(self):
        ...
        if self.tick % 6 != 0:
            return self.buffer
        Frame.copy_rect(dst=self.buffer, dst_x=0, dst_y=0, src=self.frame, src_x=self.origin[0], src_y=self.origin[1],
                        width=self.buffer.width, height=self.buffer.height)
        if self.bounce and (self.origin[0] + self.buffer.width + self.translate[0] > self.frame.width) or (
                self.origin[0] + self.translate[0] < 0):
            self.translate = (self.translate[0] * -1, self.translate[1])
        if self.bounce and (self.origin[1] + self.buffer.height + self.translate[1] > self.frame.height) or (
