    ...
    def create_frames_from_grid(self, num_cols, num_rows):
        ...
        for row_index in range(0, num_rows):
            for col_index in range(0, num_cols):
                ...
                Frame.copy_rect(dst=new_frame, dst_x=0, dst_y=0, src=self, src_x=width * col_index,
                                src_y=height * row_index, width=width, height=height, op='copy')
                frames += [new_frame]
        return frames
...
class Layer(object):
    ...
    def composite_next(self, target):
        ...
        if src is not None:
            ...
            Frame.copy_rect(dst=target, dst_x=self.target_x + self.target_x_offset,
                            dst_y=self.target_y + self.target_y_offset, src=src, src_x=0, src_y=0, width=src.width,
                            height=src.height, op=self.composite_op)
        return src
