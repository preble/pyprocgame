...
sys.path.append(sys.path[
                    0] + '/../..')  # Set the path so we can find procgame. We are assuming (stupidly?) that the
# first member is our directory.
import procgame
import pinproc
...
class Attract(game.Mode):
    ...
    def __init__(self, game):
        ...
        self.layer = dmd.ScriptedLayer(128, 32, [{'seconds': 2.0, 'layer': self.splash},
                                                 ...
                                                 {'seconds': 2.0, 'layer': self.game_title},
                                                 {'seconds': 2.0, 'layer': self.press_start},
                                                 {'seconds': 2.0, 'layer': None}])
...
class Game(game.BasicGame):
    ...
    def setup(self):
        ...
        self.trough = procgame.modes.Trough(self, trough_switchnames, 'trough6', 'trough', early_save_switchnames,
                                            'shooterR', self.drain_callback)
